  ...
  module Provider
    module Aws
      class ReservedRdsInstance < CvCollector::Provider::Aws::Base
        ...
        def fetch_data()
          data = {}
          marked_as_reserved = JSON.parse(connection.get('/v01/computes/reserved?compute_type=rds_instance&format=json', @auth_token).body)
          marked_as_reserved.map!{|x| x.inject({}){|memo,(k,v)| memo[k.to_sym] = v; memo}}
          
          REGIONS.each do |region|
            ec2 = RightAws::RdsInterface.new(@access_key_id, @secret_access_key, :endpoint_url => "https://rds.#{region}.amazonaws.com")
            reserved_instances = ec2.describe_reserved_db_instances
            _reserved_instances = []
            ...
            _reserved_instances.each do |reserved|
              reserved[:instance_count].times do 
                ...
                if _inst = marked_as_reserved.find{|instance| instance[:region] == region && 
                                                              ...
                                                              (/#{Regexp.escape(instance[:platform])}/i.match(reserved[:product_description]) || /#{Regexp.escape(reserved[:product_description])}/i.match(instance[:platform])) &&
                                                              (reserved[:multi_az] == true ?  /#{instance[:platform]}/i.match('multi_az') : true)
                                                              
                                                              }
                  marked_as_reserved.delete(_inst)
                else
                  ec2 = RightAws::RdsInterface.new(@access_key_id, @secret_access_key, :endpoint_url => "https://rds.#{region}.amazonaws.com")
                  _instances ||= ec2.describe_db_instances

                  _inst = _instances.find{|instance| instance[:instance_class] == reserved[:instance_class] && 
                                                     instance[:multi_az] == reserved[:multi_az] &&
                                                     (instance[:engine] && reserved[:product_description] && (/#{Regexp.escape(instance[:engine])}/i.match(reserved[:product_description]) || /#{Regexp.escape(reserved[:product_description])}/i.match(instance[:engine]))) && 
